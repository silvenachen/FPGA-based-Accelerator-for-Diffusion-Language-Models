...
void systolic_modulate(     //modified to multiply (1, 128) * (128, 512)
  float v217[1][128],
  float v218[128][512],
  float v219[1][512]
) {	// L322
  ...
  l_outer_tile_mi: for (int mi = 0; mi < 1; mi++) {	// L326
    l_ni: for (int ni = 0; ni < 64; ni++) {	// L327
    // #pragma HLS dataflow
...
void adaLN_modulate(
  float v236[1][128],   // input c
//   float v237[128][3072],    // adaLN_weight
  float v237[128][512],    // adaLN_weight
//   float v238[3072],
...
void allo_DDitBlock(
  ...
  float buffer4[512];
  float buffer5[512];
  float adaLN_weight[128][512];   
  float adaLN_bias[512];  
  float buffer_c[1][128];
  //for RoPE
  float buffer_sin[1024][32];
  float buffer_cos[1024][32];

  // Compute AdaLN transformation
